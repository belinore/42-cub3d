...
/*   Updated: 2025/12/02 17:15:53 by belinore         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall_slice(t_game *game, t_wall *wall)
{
	...
	tex_pos = step * (wall->top_y - game->mouse_y + wall->height / 2);
	y = wall->top_y;
	while (y < wall->bottom_y)
	{
		...
		put_pixel(&game->img, wall->column, y, color);
		y++;
	}
}
...
void	calc_wall_height_and_texture(t_game *game, t_ray *ray, int column)
{
	...
	wall = &ray->wall;
	wall->column = column;
	wall->height = (int)(game->dist_to_camera_plane / ray->perp_wall_dist);
	wall->top_y = (game->mouse_y) - (wall->height / 2);
	wall->bottom_y = (game->mouse_y) + (wall->height / 2);
	if (wall->top_y < 0)
		wall->top_y = 0;
